...
Ahead-of-time compiler input; not imported by the installed wheel runtime.
Dimensions/strides are runtime values except the fixed 256-wide head. Use cp.async:
...
@gluon.jit
def _stage_cache(k_ptr, v_ptr, k_smem, v_smem, slot, head, H_KV, D: gl.constexpr, N: gl.constexpr):
...
def q8_prefill_async_kernel(
    ...
    scale_stride_block, scale_stride_token, scale_stride_head,
    bt_stride, out_stride_t, out_stride_h, SCALE,
    H_Q, H_KV, D: gl.constexpr, PAGE,
):
    M: gl.constexpr = 16
    N: gl.constexpr = 32
...
def q8_grouped_async_kernel(q_ptr, k_ptr, v_ptr, ks_ptr, vs_ptr, tables_ptr, lengths_ptr, partial_ptr, maximum_ptr, sum_ptr,
                         Q_PER_SEQ, H_Q, H_KV, D: gl.constexpr, PAGE,
                         TABLE_WIDTH, SPLITS, SCALE):
    M: gl.constexpr = 16
    N: gl.constexpr = 32
