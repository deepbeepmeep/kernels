...
void dense_attention_partials_cuda(const void * query, const void * key_cache, const void * value_cache, bool bfloat16, const int32_t * block_tables, const int32_t * context_lens, float * partial_values, float * partial_maxima, float * partial_sums, int num_queries, int num_sequences, int query_heads, int kv_heads, int head_dim, int page_size, int num_cache_blocks, int max_blocks_per_sequence, int num_splits, float softmax_scale, cudaStream_t stream);
