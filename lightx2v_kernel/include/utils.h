...
inline at::Tensor pad_tensor(const at::Tensor& tensor, int64_t alignment = 4, bool is_column_major = false) {
  ...
  at::Tensor padding = at::zeros({pad_rows, cols}, tensor.options());
  at::Tensor tensor_padded = at::cat({tensor, padding}, 0);  // Pad along rows
